...
void QtPCSC::connect_reader(std::string reader, std::string protocol) {
    _log("PCSC: connecting to %s", reader.c_str());
    LONG err = pcsc.connect(reader, protocol);
    ...
    if (err == LONG(SCARD_E_NO_SMARTCARD)) {
        ...
        err = pcsc.wait(reader, protocol);
        emit show_insert_card(false, reader, pcsc.getContext());
    }
    if (err == SCARD_S_SUCCESS) {
        emit reader_connected(err, reader, pcsc.protocol == SCARD_PROTOCOL_T0 ? "T=0" : "T=1", pcsc.getStatus().atr);
    } else {
...
PCSCReader QtPCSC::getReader(const QString &origin)
{
    ...
    if (dialog.exec() == 0)
        throw UserCanceledError(); // TODO: exceptions
